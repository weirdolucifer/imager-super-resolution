...
from compare import compare_psnr
        ...
        with tf.Session() as sess:
            ...
            for idx in range(0, self.dataset_size, self.batch_size):
                ...
                for (i, og, nn, bl, bc, la, sr, image) in zip(range(100), original, nearest, bilinear, bicubic, lanczos, superres_imgs, images):
                    nn, _ = compare_psnr(og, og, nn)
                    bl, _ = compare_psnr(og, og, bl)
                    bc, _ = compare_psnr(og, og, bc)
                    la, _ = compare_psnr(og, og, la)
                    sr, _ = compare_psnr(og, og, sr)
                    # display(image)
                    plt.subplot(111)
